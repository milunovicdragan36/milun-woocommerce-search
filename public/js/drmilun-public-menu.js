...
class Live_Search_Menu{
  
  constructor(){
     this.searchFieldProductsMenu = jQuery(".search-term-menu");
 this.searchFieldCategoriesMenu = jQuery(".search-term-menu");
  this.searchFieldTagsMenu = jQuery(".search-term-menu");
...
this.count = 0;
 this.number_of_words_in_posts = jQuery("#numberofwordsinposts");
     ...
     this.searchFieldMenuWoo = jQuery(".search-term-menu-woo");
...
this.searchFieldProductsMenu.on(
  "keyup",
  debounce(this.getResultsProductsMenu.bind(this), 400)
);

this.searchFieldCategoriesMenu.on(
  "keyup",
  debounce(this.getResultsCategoriesMenu.bind(this), 400)
);
...
jQuery(".data-container").css("display",'none');
  ...
  jQuery('.data-posts-inc-menu').css('display','none');

 
  jQuery('.no_more_posts_menu').css("display",'none');
...
jQuery(".wrapper-data-container-menu-data-posts").css("display",'none');


 jQuery(".data-search_categories-container-menu").css("display",'none');
  jQuery(".data-tags-container-menu").css("display",'none');
...
getResultsProductsMenu(){
 
jQuery(".wrapper-data-container-menu-data-posts").css("display",'block');
  jQuery(".data-search_categories-container-menu").css("display",'block');
...
let counter = 1;
 jQuery(".search-term-menu").addClass("loadinggif");
        jQuery('.data-menu-posts-btn').css('display','block');


var find_element = 1;

var categories =this.search_categories.val();
var customSearchBox =this.searchFieldProductsMenu.val();
...
if (el) {
 ...
 jQuery(".data-categories-container-menu").css("display",'block').after("<div class='line_below_cat_tag'></div>");
...
if (result.length==0) {
  jQuery(".category-menu-label").remove();


    document.getElementsByClassName('data-categories-container-menu')[0].innerHTML =liveSearchDataCategories[0].not_found_data;
    ...
    document.getElementsByClassName('data-categories-container-menu')[0].innerHTML =result;
...
var liveSearchMenu = new Live_Search_Menu();
