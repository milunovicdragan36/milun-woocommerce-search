...
class Live_Search_Widget_Full_Width{
  
  constructor(){
     this.searchFieldProductsWidgetFullWidth = jQuery(".search-term-widget_full_width");
 this.searchFieldCategoriesWidgetFullWidth = jQuery(".search-term-widget_full_width");
  this.searchFieldTagsWidgetFullWidth = jQuery(".search-term-widget_full_width");
...
this.count = 0;
 this.number_of_words_in_posts = jQuery("#numberofwordsinposts");
     ...
     this.searchFieldWidgetWoo = jQuery(".search-term-before-loop-woo");
...
this.searchFieldProductsWidgetFullWidth.on(
  "keyup",
  debounce(this.getResultsProductsWidgetFullWidth.bind(this), 400)
);

this.searchFieldCategoriesWidgetFullWidth.on(
  "keyup",
  debounce(this.getResultsCategoriesWidgetFullWidth.bind(this), 400)
);
...
jQuery(".data-container").css("display",'none');
  ...
  jQuery('.data-posts-inc-widget_full_width').css('display','none');

 
  jQuery('.no_more_posts_widget_full_width').css("display",'none');
...
jQuery(".wrapper-data-container-widget_full_width-data-posts").css("display",'none');


 jQuery(".data-search_categories-container-widget_full_width").css("display",'none');
  jQuery(".data-tags-container-widget_full_width").css("display",'none');

  jQuery('.data-container-widget_full_width-inc').css('display','none');

  jQuery(".data-container-widget_full_width").css("display",'none');

      jQuery('.data-widget_full_width-posts-btn').css("display",'block');
...
getResultsProductsWidgetFullWidth(){
 jQuery(".search-term-widget_full_width").addClass("loadinggif");

jQuery(".wrapper-data-container-widget_full_width-data-posts").css("display",'block');
  jQuery(".data-search_categories-container-widget_full_width").css("display",'block');
...
var customSearchBox =this.searchFieldProductsWidgetFullWidth.val();
...
getResultsCategoriesWidgetFullWidth(){
const el = document.querySelector('.data-categories-container-widget_full_width');
...
if (el) {
  ...
  setTimeout(function () {
    jQuery(".data-categories-container-before-loop")
      ...
      var customSearchBox =  $this.searchFieldCategoriesWidgetFullWidth.val();
...
if (result.length==0) {
  jQuery(".category-widget_full_width-label").remove();


    document.getElementsByClassName('data-categories-container-widget_full_width')[0].innerHTML =liveSearchDataCategories[0].not_found_data;
    ...
    document.getElementsByClassName('data-categories-container-widget_full_width')[0].innerHTML =result;
...
var liveSearchBeforeLoopFullWidth= new Live_Search_Widget_Full_Width();
