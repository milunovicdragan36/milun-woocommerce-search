...
function milun_get_woo_search_results_three_words( $request ) {
global $wpdb;
$post_slug = isset( $request['s'] )
	? sanitize_text_field( wp_unslash( $request['s'] ) )
	: '';

$second_word = isset( $request['se'] )
	? sanitize_text_field( wp_unslash( $request['se'] ) )
	: '';

$third_word = isset( $request['ses'] )
	? sanitize_text_field( wp_unslash( $request['ses'] ) )
	: '';
...
$like1 = '%' . $wpdb->esc_like( $post_slug ) . '%';
$like2 = '%' . $wpdb->esc_like( $second_word ) . '%';
$like3 = '%' . $wpdb->esc_like( $third_word ) . '%';

if ( '1' === $search_by_woo_content && '1' !== $search_by_woo_title ) {
