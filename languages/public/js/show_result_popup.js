...
document.getElementsByClassName('data-container-popup')[0].innerHTML =
  livesearchposts_2.map(item => {
    ...
    const priceHTML = `<div class="search-price" style="color:#000; line-height:1em; font-size:13px;">${milunTitleFullWidth.currency_symbol}${item.price || '0'}</div>`;
    const contentHTML = item.post_content && item.post_content.trim() !== ''
      ? `<div class="content_popup" style="overflow:hidden; text-overflow:ellipsis; white-space:nowrap; font-size:12px;">${item.post_content.split(/\s+/, number_of_words_in_posts_2).join(" ")}...</div>`
      : `<div class="content_popup" style="overflow:hidden; text-overflow:ellipsis; white-space:nowrap; font-size:12px;"><em>No description available</em></div>`;
    const linkHTML = item.post_name 
      ? `<a class='red_color' href="${liveSearchDataPosts[0].site_url}product/${item.post_name}" style="font-size:12px;">${liveSearchDataPosts[0].read_more}</a>` 
      : '';
      ...
      ? `
        <div class="search-item" style="display:flex; align-items:flex-start; margin-bottom:15px; background:transparent;">
          ...
          <div class="body" style="flex:1; display:flex; flex-direction:column; justify-content:flex-start; background:transparent; height:80px; overflow:hidden;">
            ...
            <div class="content_popup" style="overflow:hidden; text-overflow:ellipsis; white-space:nowrap; font-size:12px;">
